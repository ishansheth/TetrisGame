...
bool DisplayContainer::isShapeAdded()
{
    // if shape is not moving then its added
    return !moveStatus;
}
...
bool DisplayContainer::isIntersecting(sf::Vector2f shapePosition)
{
    ...
    for(auto& s : individualComponentContainer)
    {
        ...
        for(auto& t2 : s.second)
        {
            if(
//                    bottom >= top
                shapePosition.y + SQUARE_SIDE_LENGTH + SQUARE_OUTLINE_THICKNESS  >= t2.first->getPosition().y - SQUARE_OUTLINE_THICKNESS &&
//                    top <= bottom
