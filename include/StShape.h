...
struct StShape : public IShape
{
    ...
    public:
    StShape(DisplayContainer* displayManager):
    dContainer(displayManager),
    rectangle1(new sf::RectangleShape()),
    rectangle2(new sf::RectangleShape()),
    rectangle3(new sf::RectangleShape()),
    ...
    virtual void handleKey(sf::Keyboard::Key k) override
    {
        ...
        else if(sf::Keyboard::Space == k)
        {
            ...
            vec1 += shapeCenter;
            if(isWithinDrawWindow(vec1) && isWithinDrawWindow(vec2) && isWithinDrawWindow(vec3) && isWithinDrawWindow(vec4))
            {
